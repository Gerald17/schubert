...
const WorkDayPersons = ({
			...
			render: ({ id }) => (
				<span>
					{workersReported.find(worker => worker.id === id) ? (
						...
						<>
							<Button onClick={() => handleDrawerStatus(id)}>
								Cambiar <Icon type="retweet" />
							</Button>
							<Divider type="vertical" />
							<Button
								type="danger"
								onClick={() => handleReportDrawerStatus(id)}>
								Reportar <Icon type="alert" />
							</Button>
						</>
					)}
				</span>
	...
	}, [selectedTeam, fetchWorkersByTeam]);
	...
	const handleReportSubmit = e => {
		...
		validateFields(['workerReportedStatus'], (err, values) => {
			if (!err) {
				const workerReported = workersByTeam.find(
					worker => worker.id === workerToReport
				);
	...
	const handleAddDrawerStatus = () => {
		if (!addDrawerStatus) {
			request
				.fetchData(`/api/Worker`, { journeyCreateDate })
				.then(allAvailableWorkers => {
					return allAvailableWorkers.data;
				})
				.then(substituteWorkers => {
