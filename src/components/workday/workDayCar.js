...
const WorkDayCar = ({
	...
	const handleSubmit = e => {
		...
		validateFields((err, value) => {
			if (!err) {
				const getVehicle = availableVehicles.find(
					newVehicle => newVehicle.id === value.substitute
				);
			...
			{teamVehicle && teamVehicle.hasOwnProperty('id') && (
				<>
					<Card
						...
						actions={[
							<Button onClick={() => handleDrawerStatus(teamVehicle.id)}>
								Cambiar <Icon type="retweet" />
							</Button>,
						]}>
						<Meta
							avatar={
						...
						<Card type="inner" title="Info">
							<List itemLayout="vertical">
								<List.Item>
									...
									<br />
									<Typography.Text>Km {teamVehicle.kilometers}</Typography.Text>
									<br />
									<Typography.Text>Mi {teamVehicle.mileage} </Typography.Text>
									<br />
									<Typography.Text>Estado {teamVehicle.state}</Typography.Text>
								</List.Item>
							</List>
						</Card>
					</Card>
								...
								<FormItem label="Sustituto" hasFeedback>
									{getFieldDecorator('substitute', {
										rules: [
											{
												required: true,
												message: 'Seleccione el nuevo vehículo',
											},
										],
									})(
...
export default connect(mapStateToProps, { replaceVehicle })(
